...
export default function Logo({ className = '', compact = false, markTone = 'adaptive' }) {
  const markClass = markTone === 'light' ? 'fill-white' : 'fill-brand-ink dark:fill-white';
  ...
  return (
    <Link to="/" className={`inline-flex items-center gap-3 ${className}`} aria-label="DEV TINUADE home">
      <svg className="h-10 w-10 shrink-0" viewBox="0 0 64 64" fill="none" aria-hidden="true">
        <path className={markClass} d="M9 47H20V31L9 38V47Z" />
        <path className={markClass} d="M24 47H35V17L24 24V47Z" />
