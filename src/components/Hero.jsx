...
export default function Hero() {
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(135deg,#ffffff_0%,#f5f8fc_48%,#eaf1fb_100%)] px-5 pb-16 pt-14 sm:px-6 lg:px-8 lg:pb-24 lg:pt-20">
      ...
      <div className="container-page grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
        <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75 }}>
          <div className="inline-flex items-center gap-3 rounded-md border border-brand-line bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-crisp">
            <Logo compact />
          ...
          <div className="mt-9 grid gap-3 text-sm font-semibold text-slate-700 sm:grid-cols-3">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3 lg:mt-7 lg:gap-4">
              {['Website', 'Google', 'AI'].map((label, index) => (
                <div key={label} className="rounded-lg bg-white p-4 shadow-crisp">
                  ...
                  <p className="mt-1 text-xs text-slate-500">Customer growth channel</p>
                </div>
              ))}
            </div>
          </div>
